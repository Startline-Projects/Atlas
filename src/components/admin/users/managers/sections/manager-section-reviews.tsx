...
interface ManagerSectionReviewsProps {
...
function CommentBlockView({ comment }: { comment: ManagerReviewComment }) {
  return (
    <div>
...
// admin.html lines 7784-7876 — sp-review-card (latest variant adds ink border + shadow + comments grid)
function ReviewCardView({ cycle }: { cycle: ManagerReviewCycle }) {
  const isLatest = cycle.isLatest === true;
  const hasAudit = !!(cycle.auditLabel && cycle.auditValue);
  return (
    <div
      className={cn(
    ...
    >
      ...
      {/* admin.html line 7825: sp-review-rating — flex/center + dashed bottom on latest */}
      <div
        className={cn(
          'flex items-center gap-[14px] pb-[12px] border-b border-dashed border-[var(--line-soft)] flex-wrap',
      ...
      >
        ...
        {/* Manager-tier addition (latest only): Audit score, pushed right via ml-auto */}
        {hasAudit && (
          <>
            <span
              className="font-mono text-[10px] tracking-[0.14em] uppercase text-[var(--ink-mute)] font-semibold ml-auto"
            >
              {cycle.auditLabel}
            </span>
            <span
              className={cn(
                'font-display text-[15px] font-medium tracking-[-0.01em]',
                cycle.auditHigh ? 'text-[var(--success)]' : 'text-[var(--ink)]'
              )}
            >
              {cycle.auditValue}
            </span>
...
export function ManagerSectionReviews({ profile }: ManagerSectionReviewsProps) {
  const reviews = profile.reviews;
  if (!reviews) {
    return null;
  }

  const { sectionStatus, cycles, initiateLabel, initiateAction } = reviews;

  return (
    // admin.html line 20179: <section id="mgr-section-reviews">
    <section
      id="mgr-section-reviews"
      className="border-t border-[var(--line)] py-[36px] scroll-mt-[80px] first:border-t-0 first:pt-[12px]"
    >
      {/* Section header — admin.html lines 20180-20186 */}
      ...
      {/* All cycles (latest + 4 compact past) */}
      {cycles.map((cycle, idx) => (
        <ReviewCardView key={idx} cycle={cycle} />
      ))}

      {/* admin.html lines 20262-20267 — Initiate review button (cd-action-btn warn) */}
      {initiateLabel && (
        <div className="mt-[14px]">
          <button
            type="button"
            data-mgr-action={initiateAction}
            onClick={() => {
              if (typeof window !== 'undefined') {
                // eslint-disable-next-line no-console
                console.log(`[manager-reviews] ${initiateAction} clicked: ${initiateLabel}`);
              }
            }}
