...
interface SpecialistSectionAuditProps {
  profile: SpecialistProfile;
}

type AuditLog = NonNullable<SpecialistProfile['auditLog']>;
type AuditDay = AuditLog['days'][number];
type AuditEvent = AuditDay['events'][number];
type AuditDetailSegment = AuditEvent['details'][number];

// admin.html lines 3785-3791 — data-cat dot color overrides
function dotBgClass(cat: AuditEvent['cat']): string {
  switch (cat) {
    case 'suspension':
    case 'ban':
...
function tagVariantClass(variant: AuditEvent['tagVariant']): string {
  switch (variant) {
    case 'signin':
...
function outcomeVariantClass(variant: AuditDetailSegment['outcomeVariant']): string {
  switch (variant) {
    case 'partial':
      return 'bg-[var(--amber-bg)] text-[var(--amber)]';
    case 'escalated':
...
// admin.html lines 3814-3856 — timeline-detail segments
function DetailSegment({ seg }: { seg: AuditDetailSegment }) {
  if (seg.type === 'sep') {
    // admin.html line 3841: meta-sep — line-strong
    return <span className="text-[var(--line-strong)]">{seg.text}</span>;
  }
  if (seg.type === 'refId') {
    // admin.html line 3822: ref-id — mono 11px ink-soft cream-deep chip
    return (
  ...
  if (seg.type === 'outcome') {
    // admin.html line 3842: outcome — inline-flex 10.5px uppercase
    return (
      <span
        className={cn(
...
function TimelineEntry({ event, isLast }: { event: AuditEvent; isLast: boolean }) {
  return (
    ...
    >
      {/* admin.html line 3766: ::before — vertical track line at left:84px */}
      <span
        aria-hidden="true"
        className="absolute left-[84px] top-0 bottom-0 w-[1px] bg-[var(--line-soft)] max-[540px]:hidden"
      />
      {/* admin.html line 3775: ::after — 7px dot at left:81px top:18px with paper outer ring */}
      <span
        aria-hidden="true"
        className={cn(
      ...
      {/* admin.html line 3793: timeline-time — mono 10.5px right-aligned, multi-line preserved */}
      <span
        className="font-mono text-[10.5px] text-[var(--ink-mute)] tracking-[0.04em] font-medium pt-[3px] text-right whitespace-pre-line max-[540px]:text-left max-[540px]:pt-0"
      >
        {event.time}
      </span>

      {/* admin.html line 3802: timeline-content — pl-14 */}
      <div className="pl-[14px] min-w-0 max-[540px]:pl-0 max-[540px]:col-span-full">
        {/* admin.html line 3803: timeline-action — 13.5px ink */}
        <div className="text-[13.5px] text-[var(--ink)] leading-[1.4] mb-[4px]">
          {/* admin.html line 3809: verb — weight 500 */}
          <span className="font-medium">{event.verb}</span>
          {event.target && (
            <>
              {' · '}
              {/* admin.html line 3810: target — ink semibold */}
              <span className="text-[var(--ink)] font-semibold">{event.target}</span>
            </>
          )}
        </div>
        {/* admin.html line 3814: timeline-detail — 12px mute flex-wrap */}
      ...
      {/* admin.html line 3858: timeline-tag — mono 9px uppercase pill */}
      <span
        className={cn(
...
export function SpecialistSectionAudit({ profile }: SpecialistSectionAuditProps) {
  const auditLog = profile.auditLog;
  if (!auditLog) {
    return null;
  }

  const { sectionStatus, days, footerLabel, loadMoreLabel, emptyState } = auditLog;
  const totalEvents = days.reduce((sum, d) => sum + d.events.length, 0);

  return (
    // admin.html line 19145: <section id="sp-section-audit">
    <section
      id="sp-section-audit"
      className="border-t border-[var(--line)] py-[36px] scroll-mt-[80px] first:border-t-0 first:pt-[12px]"
    >
      {/* Section header — admin.html lines 19146-19152 */}
      ...
      {totalEvents === 0 && emptyState ? (
        ...
        <>
          {/* admin.html line 19154: .timeline wrapper */}
          <div className="bg-[var(--paper)] border border-[var(--line)] rounded-[var(--r-md)] overflow-hidden">
            {days.map((day, dayIdx) => {
              const isLastDay = dayIdx === days.length - 1;
              return (
                <Fragment key={dayIdx}>
                  {/* admin.html line 3731: timeline-day-header */}
                  <div
                    className={cn(
            ...
            {/* admin.html lines 19286-19289: timeline-footer */}
            {(footerLabel || loadMoreLabel) && (
              <div className="flex items-center justify-between flex-wrap gap-[10px] px-[18px] py-[14px] border-t border-[var(--line-soft)] text-[12px] text-[var(--ink-mute)]">
                ...
                {loadMoreLabel && (
                  <button
                    type="button"
                    data-sp-action="load-more-audit"
                    onClick={() => {
                      if (typeof window !== 'undefined') {
                        // eslint-disable-next-line no-console
                        console.log('[specialist-audit] load-more clicked');
                      }
                    }}
