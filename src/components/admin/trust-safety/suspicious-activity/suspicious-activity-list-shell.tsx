...
import {
  SUSPICIOUS_ACTIVITY_ROWS,
  SUSPICIOUS_ACTIVITY_PAGE_HEADER,
  SUSPICIOUS_ACTIVITY_SEVERITY_CARDS,
  SUSPICIOUS_ACTIVITY_FILTER_CHIPS,
  SUSPICIOUS_ACTIVITY_STREAM_BAR,
  SUSPICIOUS_ACTIVITY_LIST_FOOTER,
} from '@/lib/mock-data/admin/suspicious-activity-data';
import { SuspiciousActivityLiveFeed } from './suspicious-activity-live-feed';

const SEVERITY_BORDER_TOP: Record<string, string> = {
  high: 'border-t-[var(--amber)]',
  medium: 'border-t-[var(--lime-deep)]',
  low: 'border-t-[var(--success)]',
  critical: 'border-t-[var(--danger)]',
};

export function SuspiciousActivityListShell() {
    ...
    <>
      ...
      <div className="max-w-[1480px] mx-auto px-[28px] py-[24px]">
        ...
        <div className="flex items-end justify-between gap-[18px] mb-[22px] pb-[18px] border-b border-[var(--line)] flex-wrap">
          <div className="flex-1 min-w-0">
            <h1 className="font-display text-[32px] font-medium tracking-[-0.02em] mb-1 leading-[1.1] text-[var(--ink)]">
              {SUSPICIOUS_ACTIVITY_PAGE_HEADER.title}
            </h1>
            <div className="font-mono text-[11px] tracking-[0.04em] text-[var(--ink-mute)] flex flex-wrap items-center gap-2">
              <span>{SUSPICIOUS_ACTIVITY_PAGE_HEADER.meta}</span>
              <span className="inline-flex items-center gap-[5px] ml-[12px] text-[var(--amber)] font-semibold">
                {SUSPICIOUS_ACTIVITY_PAGE_HEADER.newIndicator}
              </span>
            </div>
          </div>
          <div className="inline-flex gap-[8px] items-center flex-wrap flex-shrink-0">
            <span className="inline-flex items-center gap-[6px] px-[10px] py-[5px] bg-[var(--success-bg)] text-[var(--success)] font-mono text-[10px] tracking-[0.12em] uppercase font-bold rounded-full border border-[rgba(74,109,65,0.25)]">
              <span
                className="w-[6px] h-[6px] rounded-full bg-[var(--success)]"
                style={{ animation: 'sa-live-pip-pulse 1.4s ease-in-out infinite' }}
              />
              {SUSPICIOUS_ACTIVITY_PAGE_HEADER.liveIndicator}
            </span>
            <button
              type="button"
        ...
        <div className="grid grid-cols-4 max-[900px]:grid-cols-2 max-[600px]:grid-cols-1 gap-[10px] mb-[22px]">
          {SUSPICIOUS_ACTIVITY_SEVERITY_CARDS.map((card) => (
            <div
              key={card.id}
            ...
            >
              ...
              <div className="font-mono text-[10px] text-[var(--ink-mute)] tracking-[0.02em]">
                {card.id === 'high' ? (
                  <>
                    <strong className="text-[var(--ink)] font-semibold">3 unreviewed</strong> · last 24h · auto-pages on s5
                  </>
                ) : (
                  card.meta
                )}
        ...
        {/* Stream control bar */}
        <div className="flex items-center justify-between p-[10px_14px] bg-[var(--paper)] border border-[var(--line)] rounded-[var(--r-sm)] mb-4 flex-wrap gap-[10px]">
          <div className="inline-flex items-center gap-[12px] flex-wrap font-mono text-[11px] text-[var(--ink-soft)] tracking-[0.02em]">
            <span>
              <strong className="text-[var(--ink)] font-semibold">Last event:</strong> {SUSPICIOUS_ACTIVITY_STREAM_BAR.lastEvent}
            </span>
            <span className="text-[var(--line-strong)]">·</span>
            <span>
              <strong className="text-[var(--ink)] font-semibold">Avg rate:</strong> {SUSPICIOUS_ACTIVITY_STREAM_BAR.avgRate}
            </span>
            <span className="text-[var(--line-strong)]">·</span>
            <span>
              <strong className="text-[var(--ink)] font-semibold">Auto-actions today:</strong>{' '}
              <span className="text-[var(--success)] font-bold">+{SUSPICIOUS_ACTIVITY_STREAM_BAR.autoActions.dismissed} dismissed</span>
              <span> · </span>
              <span className="text-[var(--amber)] font-bold">{SUSPICIOUS_ACTIVITY_STREAM_BAR.autoActions.watchlisted} watchlisted</span>
            </span>
          </div>
          <div className="inline-flex gap-[6px]">
            <button
              ...
              className={`inline-flex items-center gap-[6px] px-[12px] py-[5px] font-mono text-[11px] font-semibold border rounded-full cursor-pointer tracking-[0.04em] transition-all ${
                isPaused
            ...
            <button
              type="button"
              className="inline-flex items-center gap-[6px] px-[12px] py-[5px] font-mono text-[11px] font-semibold text-[var(--ink-soft)] bg-[var(--paper-deep)] border border-[var(--line)] rounded-full cursor-pointer tracking-[0.04em] hover:text-[var(--ink)] hover:border-[var(--line-strong)] transition-all"
            >
              <svg
                width="11"
                height="11"
        ...
        <div className="flex items-center justify-between gap-[14px] mb-4 flex-wrap">
          <div className="inline-flex items-center gap-[10px] flex-wrap">
            <div className="relative inline-flex items-center">
              ...
              <input
                type="text"
                placeholder="Search by event ID, account, or activity type…"
                value={searchQuery}
            ...
            <div className="inline-flex gap-[6px] flex-wrap">
              {SUSPICIOUS_ACTIVITY_FILTER_CHIPS.map((chip) => {
          ...
          <button
            type="button"
            className="inline-flex items-center gap-[6px] px-[14px] py-[8px] font-body text-[12.5px] font-medium bg-[var(--paper)] border border-[var(--line)] rounded-full text-[var(--ink-soft)] cursor-pointer whitespace-nowrap hover:border-[var(--ink)] hover:text-[var(--ink)] hover:bg-[var(--cream-deep)] transition-all flex-shrink-0"
          >
            <svg
              width="13"
              height="13"
        ...
        {/* Live feed table */}
        <SuspiciousActivityLiveFeed rows={SUSPICIOUS_ACTIVITY_ROWS} />
