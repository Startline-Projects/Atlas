/**
 * Phase 15a — Fraud alerts toolbar (search + 5 filter chips + date picker).
 *
 * admin.html CSS: .fr-toolbar + .fr-filter-chip (L15504-15560)
 * admin.html markup: L39249-39280
 *
 * Layout: flex justify-between. Left = search + filter chips. Right = date picker.
 * Search: body 13px, py-8 pl-34 pr-14, bg paper-deep, rounded-full, w-280.
 * Chip: body 12px 500, ink-mute, bg paper-deep, border line, rounded-full.
 * Active: bg-ink, paper, border-ink, 600. chip-count active: rgba(251,248,242,0.18).
 */
import type { FraudFilterKey, FraudFilterChip } from '@/lib/mock-data/admin/fraud-alerts-data';

interface FraudAlertsToolbarProps {
  searchQuery: string;
  onSearchChange: (q: string) => void;
  selectedFilter: FraudFilterKey;
  onFilterChange: (f: FraudFilterKey) => void;
  ...
  return (
    <div className="flex items-center justify-between gap-[14px] mb-[16px] flex-wrap">
      ...
      <div className="inline-flex items-center gap-[10px] flex-wrap">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search by user, alert ID, type, or pattern…"
      ...
      <div>
        <button
          type="button"
          data-fraud-action="open-date-picker"
          onClick={() => console.log('[fraud] date-picker clicked')}
