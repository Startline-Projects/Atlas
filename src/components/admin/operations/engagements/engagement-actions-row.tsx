...
interface EngagementActionsRowProps {
  engagementId: string;
}
...
// admin.html lines 21717-21741 — 7 buttons + 1 divider
const ACTIONS_BEFORE: ActionConfig[] = [
  {
    key: 'message-both',
    label: 'Message both parties',
    variant: 'lime',
    icon: (
  ...
  },
  {
    ...
    icon: (
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        <path d="M2 12h2l3-9 4 18 3-9h8" />
      </svg>
    ),
  },
  {
    key: 'force-payment',
    label: 'Force payment release',
    variant: 'warn',
    icon: (
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
function ActionButton({ action, engagementId }: { action: ActionConfig; engagementId: string }) {
  return (
    <button
      type="button"
      data-eng-action={action.key}
      onClick={() => {
        // eslint-disable-next-line no-console
        console.log(`[engagement-action] ${action.key} clicked for ${engagementId}`);
      }}
      className={cn(
...
export function EngagementActionsRow({ engagementId }: EngagementActionsRowProps) {
  return (
    // admin.html line 21715: eng-actions-row (standalone card with bg-paper + border + r-md)
    <div className="flex flex-wrap gap-[8px] items-center mb-[22px] py-[14px] px-[18px] bg-[var(--paper)] border border-[var(--line)] rounded-[var(--r-md)]">
      ...
      {ACTIONS_BEFORE.map((action) => (
        <ActionButton key={action.key} action={action} engagementId={engagementId} />
      ))}
