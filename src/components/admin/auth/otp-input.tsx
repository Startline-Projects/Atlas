      ...
      <div className="flex justify-between gap-2">
        {values.map((value, index) => (
          <input
            ...
            className={`w-full h-[60px] text-center font-display text-[28px] font-medium text-[var(--color-ink)] bg-[#FFFDF7] border border-[var(--color-line)] rounded-[var(--radius-md)] transition-all focus:outline-none ${
              error
                ? 'border-[var(--color-danger)] bg-[var(--color-danger-bg)] text-[var(--color-danger)] animate-shake'
                : 'focus:border-[var(--color-ink)] focus:shadow-[0_0_0_3px_rgba(14,14,12,0.08)]'
            } ${value ? 'border-[var(--color-ink-soft)]' : ''}`}
          />
        ))}
      </div>
      {error && (
        <div className="flex items-start gap-2 mt-4 text-sm text-[var(--color-danger)]">
