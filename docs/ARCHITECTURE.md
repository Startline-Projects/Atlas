...
### 7.1 Authentication
Supabase Auth (ADR 0001 — `docs/adr/0001-supabase-auth.md`). The session is a Supabase access token in an HttpOnly cookie, one cookie pair per surface (`atlas_session`+`atlas_refresh` for candidates, `atlas_admin_session`+`atlas_admin_refresh` for admins, `atlas_client_session`+`atlas_client_refresh` for clients). Route handlers read it via `requireCandidateSession()` / `requireAdminSession()` / `requireClientSession()` (401 when absent); Server Components via `getCandidateSession()` / `getAdminSession()` / `getClientSession()` (redirect when null); all are memoised per request. The session object carries `userId`, `email`, and the resolved domain user (`candidate` / `admin` / `client`). The Supabase mechanics shared by the self-registering surfaces (create auth user, verify code, sign in with lockout, revoke, and the dev OTP bypass) live once in `lib/services/auth-user`; each role service keeps its own `User`+profile pair in step with them. Services receive `session.userId` (or the domain user) as an argument; they never read cookies or request context. `src/proxy.ts` is the outer gate: cookie-presence redirect, access-token refresh (the only place cookies can be rewritten before a render), and per-IP rate limits — validity is always re-checked by the layout/page/route.
